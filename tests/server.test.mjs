...
test('Vercel can load the entrypoint before starting its captured server', { timeout: 10000 }, async (t) => {
    ...
    const response = await fetch(`${base}/translations/zh-Hans/manifest.json`)
    assert.equal(response.status, 200)
    assert.equal(await response.text(), await readFile(new URL('../translations/zh-Hans/manifest.json', import.meta.url), 'utf8'))
    const home = await fetch(base, { redirect: 'manual' })
    assert.equal(home.status, 200)
    assert.deepEqual(await home.json(), { service: 'Game Translation', translations: '/translations/' })
